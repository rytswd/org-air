...
;; (draft/ready/work-in-progress/complete/dropped), tags and a title; this
;; module reads them, groups by state / directory / tag, and draws a
;; box-tree with state badges, version/subfolder nesting and per-folder
;; roll-up counts.  The tree is buffer TEXT so it is fully byte-testable;
;; the state badge degrades from emoji to a `[D]'-style label off a
;; graphical frame.
...
(defcustom org-air-project-states
  ...
  "Air doc states in display order (top-level boxes when grouping by state)."
  :type '(repeat string)
  :group 'org-air)
...
(defun org-air-project--tree (name)
  "Return tree glyph NAME via the shared org-air glyph table."
...
(defun org-air-project--dir-node ()
  "Return a fresh directory node: (DOCS . CHILD-ALIST)."
  (cons nil nil))

(defun org-air-project--insert-into-tree (root-node doc components)
  "Insert DOC under COMPONENTS (a list of directory names) of ROOT-NODE."
  (if (null components)
      (setcar root-node (cons doc (car root-node)))
    (let* ((head (car components))
           (child (assoc head (cdr root-node))))
      (unless child
        (setq child (cons head (org-air-project--dir-node)))
        (setcdr root-node (append (cdr root-node) (list child))))
      (org-air-project--insert-into-tree (cdr child) doc (cdr components)))))

(defun org-air-project--build-dir-tree (docs)
  "Return a directory tree node grouping DOCS by their relpath folders."
  (let ((root (org-air-project--dir-node)))
    (dolist (doc docs)
      (let ((dirs (butlast (split-string (org-air-doc-relpath doc) "/"))))
        (org-air-project--insert-into-tree root doc dirs)))
    root))

(defun org-air-project--node-descendant-count (node)
  "Return the number of docs in NODE's descendant folders (not direct)."
  (apply #'+ (mapcar (lambda (child)
                       (+ (length (car (cdr child)))
                          (org-air-project--node-descendant-count (cdr child))))
                     (cdr node))))
...
(defun org-air-project--insert-dir-node (node indent show-badge)
  "Insert the folder NODE's docs and child folders at INDENT (F5d).
SHOW-BADGE prefixes each doc row with its state badge (directory/tag
groupings, where a box mixes states)."
  ;; Direct docs first.
  (dolist (doc (reverse (car node)))
    (org-air-project--insert-doc-row doc (1+ indent) show-badge))
  ;; Then child folders, each as a branch with a count + roll-up.
  (let ((children (cdr node))
        (i 0))
    (dolist (child children)
      (setq i (1+ i))
      (let* ((last (= i (length children)))
             (name (car child))
             (sub (cdr child))
             (direct (length (car sub)))
             (descend (org-air-project--node-descendant-count sub))
             (branch (concat (org-air-project--tree 'box-vertical)
                             (make-string (max 0 (1- indent)) ?\s)
                             (org-air-project--tree
                              (if last 'box-bottom-left 'box-tee-left))
                             (org-air-project--tree 'box-horizontal) " "))
             (count (if (> descend 0)
                        (format "%d (+%d)" direct descend)
                      (format "%d" direct))))
        (insert (propertize branch 'face 'org-air-face-air-tree)
                (propertize (concat name "/") 'face 'org-air-face-section)
                "    "
                (propertize count 'face 'org-air-face-count)
                "\n")
        (org-air-project--insert-dir-node sub (+ indent 3) show-badge)))))
...
row with its own state badge."
  (let* ((width org-air-project--width)
    ...
    (insert (propertize (org-air-project--tree 'box-vertical)
            ...
            "\n")
    (org-air-project--insert-dir-node tree 0 show-badge)
    (insert (propertize
             (concat (org-air-project--tree 'box-bottom-left)
                     (make-string (max 0 (- width 1))
                                  (aref (org-air-project--tree 'box-horizontal) 0)))
             'face 'org-air-face-air-tree)
            "\n")))

(defun org-air-project--group-by-state (docs)
  "Return DOCS grouped by state as a list of (TITLE BADGE FACE DOCS SHOW)."
  (delq nil
        (mapcar
         (lambda (state)
           (let ((members (seq-filter
                           ...
                           docs)))
             (when members
               (list (org-air-project--state-title state)
                     (org-air-project--state-badge state)
                     (org-air-project--state-face state)
                     members nil))))
         (append org-air-project-states
                 (seq-uniq (seq-remove
                            (lambda (s) (member s org-air-project-states))
                            (mapcar #'org-air-doc-state docs)))))))

(defun org-air-project--group-by-directory (docs)
  "Return DOCS grouped by their top-level directory."
  (let ((dirs (seq-uniq
               (mapcar (lambda (d)
                         (car (split-string (org-air-doc-relpath d) "/")))
                       docs))))
    (mapcar (lambda (dir)
              (list (concat dir "/") nil nil
                    (seq-filter
                     (lambda (d)
...
(defun org-air-project--group-by-tag (docs)
  "Return DOCS grouped by tag (a doc may appear under several tags)."
  (let ((tags (seq-uniq (apply #'append (mapcar #'org-air-doc-tags docs)))))
    (mapcar (lambda (tag)
              (list (concat "#" tag) nil nil
                    (seq-filter (lambda (d) (member tag (org-air-doc-tags d)))
                                docs)
                    t))
            (sort tags #'string-lessp))))

(defun org-air-project--group (docs)
  "Return DOCS grouped per `org-air-project-group'."
  (pcase org-air-project-group
    ('directory (org-air-project--group-by-directory docs))
    ('tag (org-air-project--group-by-tag docs))
    (_ (org-air-project--group-by-state docs))))
...
(defun org-air-project--render (root)
  ...
  (let* ((inhibit-read-only t)
         ...
         (groups (org-air-project--group docs)))
    (erase-buffer)
    ...
    (if (null docs)
        (insert "  "
                ...
                "\n")
      (dolist (g groups)
        (pcase-let ((`(,title ,badge ,face ,members ,show) g))
          (org-air-project--insert-group-box title badge face members show)
          (insert "\n"))))
    (goto-char (point-min))
