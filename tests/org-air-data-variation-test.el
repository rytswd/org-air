...
Ground truth from the fidelity review: items on 8/10/12/15/16/17/18/20;
15 is today (▮), so ● must be exactly 8/10/12/16/17/18/20 — F1's
literals drop 8/10/18 and fabricate 19."
  (skip-unless (locate-library "org-air"))
  (org-air-viewport-test-as-gui
    (org-air-viewport-test-with-dashboard 120
      (pcase-let ((`(,marked . ,today) (org-air-viewport-test-calendar-marks)))
        (should (equal marked '(8 10 12 16 17 18 20)))
        (should (equal today '(15)))))))
