...
(defvar org-air-test-known-failures
  '(
    ...
    ;;
    ;; ===================================================================
    ;; v0.5 ROUND-80 CLOSEOUT (impl tswzkynqrpwt + test re-bless <this
    ;; change>).  First-class OUT/OFF Air states across TWO surfaces,
    ;; REUSING the pill/badge machinery (air/v0.1/org-air-round80-
    ;; design.org).  ALL 15 flagged goldens RE-BLESSED HONESTLY and
    ;; DELETED from this manifest (a passing listed test is itself a gate
    ;; failure), so the R80 manifest is EMPTY:
    ;;   (a) KEYWORD-BADGE MIN-WIDTH (D2): a board whose widest SHOWN
    ;;       keyword was < 5 cols (all TODO/DONE, 4) re-freezes its keyword
    ;;       column ONE cell wider (floored to `org-air-keyword-badge-min-
    ;;       cols' = 5), so every board title/pill shifts +1 col.  The 28
    ;;       board byte goldens (layout-mockup-* tiers + board-only-70 +
    ;;       denote-origin-{80,120}) regenerated via `make regen-mockups'
    ;;       (FROZEN-CLOCK renderer, guards active; jj diff = ONLY the
    ;;       keyword-cell +1 shift, every rail/calendar/day line byte-
    ;;       identical; empty board + review + entry-view UNCHANGED because
    ;;       tw-todo=0 there makes the floor a no-op).  The assertion
    ;;       goldens re-blessed: r13-priority-slot (`TODO  ■ ' / 4-space
    ;;       plain lead), r21-4-keyword-badge-text-fallback (the min-width-
    ;;       PADDED token `NEXT ', no display image), r49-5 (matches the
    ;;       regenerated 120 mockup).
    ;;   (b) STATE-REGISTRATION (D1): out/off join every project state list
    ;;       after `complete', before `dropped'.  Goldens re-blessed: the
    ;;       three project-view-{state,dir,tag} byte mockups (+Out/+Off
    ;;       count lines, regenerated); r20-5-state-display-order (the
    ;;       DELIBERATE airctl forward-divergence, PRODUCT-CONFIRM);
    ;;       r25-3-{display-order,five-real-states} (7-entry literals);
    ;;       r26-3-legend-on-screen-popped (the Summary grew 2 lines, so
    ;;       the pathological fold heights bump +2 — body 26 for the header,
    ...
    ;;
    ;; ===================================================================
    ;; v0.1 ROUND-82 CLOSEOUT (impl tip xrnqkqkp + test seat <this
    ;; commit>).  USER REQUEST: the `e' editor's `,' priority field,
    ;; reworked by R76 into a full-range one-key read-char PICKER
    ;; (`read-char-exclusive' + prompt "Priority %c-%c, SPC clears[, RET
    ;; keeps %c]:", the user types an explicit letter), is ANNOYING —
    ;; "just cycle through instead".  R82 DROPS R76's prompt and keeps
    ;; its reachability + real clear: `,' is now a forward-WRAPPING cycle
    ;; (none -> A -> B -> ... -> E -> none -> ...) over the write
    ;; target's own range, ONE slot per press, NO `read-char'/minibuffer
    ;; (impl: the picker-only `org-air-inbox--read-priority-char' +
    ;; `--priority-normalize' RETIRED, REPLACED by the PURE
    ;; `org-air-inbox--priority-cycle-next (current range)'; the suffix
    ;; body swapped picker -> cycle; `,' binding / name / :transient t /
    ;; description lambda KEPT; ZERO apply-leg + ZERO preview/field-row
    ;; edits — the tri-state `?\s' sentinel rides both legs unchanged).
    ;;
    ;; The 14 R76 priority ERTs pinned the RETIRED picker and were WRONG
    ;; by design.  The test seat SUPERSEDED them per the spec's Test-plan
    ;; call-out: round76-test.el (every one of r76-1..r76-14 drove the
    ;; now-absent read-char picker — `read-char-exclusive', the pure
    ;; `--priority-normalize', the "A-E"/"RET keeps"/"must be between"
    ;; prompt strings) is DELETED whole, and the still-valid coverage is
    ;; re-driven by CYCLE presses in the new tests/org-air-round82-test.el
    ;; (r82-1..r82-9).  The preserved intent: r76-9's both-leg apply +
    ;; r76-4's tri-state clear (byte-unchanged apply path) -> r82-8;
    ;; r76-10's WYSIWYG preview/field tri-state -> r82-4; r76-5's
    ;; cookie-less safe no-op -> r82-9.  The new cycle seams run every
    ;; press under a NO-READ GUARD (`read-char'/`read-char-exclusive'/
    ;; `read-string' `cl-letf'd to a raising stub — a surviving R76
    ;; prompt reddens): r82-1 C->D one press no-read (the ask); r82-2
    ;; pure ring incl. out-of-range restart; r82-3 wrap LOW->none->A
    ;; (up-reachability, no prompt); r82-4 live preview/row per press;
    ;; r82-5 out-of-range/none/cookie-less starts; r82-6 write-target
    ;; range governs the wrap; r82-7 one-letter none<->B toggle; r82-8
    ;; clear+set apply both legs + ring; r82-9 cookie-less none writes
    ;; nothing, no error.  ZERO byte goldens moved (the transient renders
    ;; in no mockup/golden — `make regen-mockups' byte-clean, as R76/R70/
    ...
    ;; manifest is EMPTY.
    ;; ===================================================================
    ;; v0.1 ROUND-88 CLOSEOUT (impl tip qwpqnqmouzmq + test re-bless <this
    ;; commit>).  ALL 4 R88 grind entries CLOSED on the test seat.  The
    ;; date cell became a five-level PROXIMITY HEAT-RAMP
    ;; (air/v0.1/org-air-round88-design.org): the impl widened
    ;; `org-air-view--day-relative-face' from {0->today, 1->tomorrow, else
    ;; nil} to {0->day-today ORANGE, 1->day-tomorrow BLEND, 2..6->day-week
    ;; AMBER (NEW), <0 and >=7 -> nil}, recoloured the two day faces
    ;; (teal->orange today, rose->the per-channel midpoint blend tomorrow),
    ;; ADDED `org-air-face-day-week' and gave `org-air-face-overdue' an
    ;; explicit R79 spec RED in BOTH tiers (dark ALIGNED Nord11 #BF616A, was
    ;; the inherited `critical'/Nord13 YELLOW that INVERTED the ramp in dark
    ;; mode).  A FACE-ONLY change: the label BYTES are untouched, so EVERY
    ;; text golden is byte-identical (verified: `make regen-mockups' is a
    ;; no-op, the full suite moved ZERO goldens); only the GUI pill PIXELS
    ;; repaint.  R87 rule A and the svg-pill `:fill (face-foreground FACE)'
    ;; mechanism are UNCHANGED (zero arm edits); no rescan, no
    ;; `org-air-view--cache-version' bump (R53).
    ;;
    ;; The 4 flagged assertions RE-BLESSED to the ramp reality (in place,
    ;; permanent regression guards):
    ;;   r85-1  now asserts the 5-level map (0->today, 1->tomorrow,
    ;;     2..6->day-week, -1 & >=7 -> nil) — the +2 line was "-> nil".
    ;;   r85-7  a +3d neutral date -> `org-air-face-day-week' (was
    ;;     `org-air-face-date'; label bytes "Thu" unchanged).
    ;;   r87-7  SPLIT at the week boundary — a delta-3 deadline/scheduled
    ;;     -> `org-air-face-day-week', a delta-7 (BEYOND) keeps its slot
    ;;     face (was the flat "delta>=2 keeps slot").
    ;;   r87-10 the delta-3 sub-line of the notes-arm guard ->
    ;;     `org-air-face-day-week' (its today/tomorrow half UNCHANGED).
    ;; r85-3/4/8/9/14 + r87-1..6/8..15 STAND verbatim (SYMBOL/byte-level;
    ;; the day faces are RECOLOURED, not renamed).  The 15 new r88-1..15
    ;; seams (tests/org-air-round88-test.el) landed as permanent guards:
    ;; r88-1..13 the design §ERT ledger (overdue-red-both-tiers, the 5
    ;; buckets on all 3 arms, the 6-vs-7 boundary, distinct symbols +
    ;; hexes, the tomorrow blend, text+pill reach, byte layer, past-neutral
    ;; guard, R79 convention) + r88-14 (the align is surgical — critical
    ;; UNTOUCHED) + r88-15 (R53 data-purity for the amber band).  Round-88
    ;; manifest is EMPTY.
    ;; ===================================================================
    ;; v0.1 ROUND-90: the three design-obsolete R83 cold-row assumptions
    ;; were re-blessed for header-only Backlog + explicit reveal.  Empty.
    )
